-- =============================================
-- Safe Migration Script for Complete Hotel System Update
-- Includes Room Categories + Complete Room Inventory with Hit Column
-- September 22, 2025 
-- =============================================
...
-- =============================================
CREATE TABLE IF NOT EXISTS hotel_rooms_backup_20250922 AS 
SELECT * FROM hotel_rooms WHERE 1=0; -- Create empty backup table structure

-- =============================================
-- STEP 2: Create/Update Room Categories Table
-- =============================================
...
-- Insert/Update room categories with current rates
...
-- =============================================
-- STEP 3: Backup existing room data if table exists
-- =============================================
SET @table_exists = (
    SELECT COUNT(*) 
...
SET @backup_sql = IF(@table_exists > 0, 
    'INSERT INTO hotel_rooms_backup_20250922 SELECT * FROM hotel_rooms', 
    'SELECT "No existing hotel_rooms table to backup" as message'
);
...
-- =============================================
-- STEP 4: Drop and recreate hotel_rooms table with Hit column
-- =============================================
...
('109', 'DLX', 1, 'NO', 'Yes', 'YA', 23), -- Note: This might be a duplicate with room 109 STD, need to verify
('107', 'STD', 1, 'NO', 'No', 'YA', 33),

-- Floor 2 Rooms (20 rooms)
...
-- =============================================
-- STEP 6: Update category_id relationships
-- =============================================
...
-- =============================================
-- STEP 7: Create/Update room availability view
-- =============================================
...
-- =============================================
-- STEP 8: Verification and Rollback Safety
-- =============================================

-- Verify data integrity
SET @room_count = (SELECT COUNT(*) FROM hotel_rooms);
SET @category_count = (SELECT COUNT(*) FROM room_categories);
SET @view_count = (SELECT COUNT(*) FROM room_availability);

-- Check if migration was successful
SELECT 
    CASE 
        WHEN @room_count = 134 AND @category_count = 6 AND @view_count = 134 
        THEN 'MIGRATION SUCCESSFUL' 
        ELSE 'MIGRATION FAILED - CHECK DATA'
    END as migration_status,
    @room_count as total_rooms_inserted,
    @category_count as total_categories,
    @view_count as rooms_in_view;

-- Show summary statistics
SELECT 
    'SUMMARY AFTER MIGRATION' as info,
    COUNT(*) as total_rooms,
    COUNT(DISTINCT room_type) as room_types,
    COUNT(DISTINCT floor_number) as total_floors,
    SUM(hit_count) as total_hits,
    ROUND(AVG(hit_count), 2) as average_hits_per_room,
    MAX(hit_count) as highest_hit_count,
    MIN(hit_count) as lowest_hit_count
FROM hotel_rooms;

-- Room type distribution with Hit statistics
SELECT 
    'ROOM TYPE STATS' as category,
    room_type,
    COUNT(*) as room_count,
...
SELECT 
    ...
    '134 hotel rooms added with Hit tracking' as details,
    'Room categories and pricing updated' as categories,
    'Room availability view created' as views,
    'Backup table created: hotel_rooms_backup_20250922' as backup,
    NOW() as completed_at;