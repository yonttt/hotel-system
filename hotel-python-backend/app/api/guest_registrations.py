...
def create_guest_registration(
    ...
    existing_registration = db.query(GuestRegistration).filter(
        GuestRegistration.registration_no == registration.registration_no
    ).first()
    ...
    db_registration = GuestRegistration(**registration_data)
...
def get_guest_registrations(
    ...
    registrations = db.query(GuestRegistration).offset(skip).limit(limit).all()
...
def get_guest_registration(
    ...
    registration = db.query(GuestRegistration).filter(GuestRegistration.id == registration_id).first()
    if not registration:
...
def get_guest_registration_by_number(
    ...
    registration = db.query(GuestRegistration).filter(GuestRegistration.registration_no == registration_no).first()
    if not registration:
        raise HTTPException(status_code=404, detail="Guest registration not found")
    return registration
...
def get_next_registration_number(
    ...
    latest_registration = db.query(GuestRegistration).order_by(GuestRegistration.id.desc()).first()
    
    if latest_registration and latest_registration.registration_no:
        try:
            # Extract number from the registration number (assuming format like "REG00000001")
            last_number = int(latest_registration.registration_no.replace("REG", ""))
    ...
    # Format as REG followed by 8 digits
